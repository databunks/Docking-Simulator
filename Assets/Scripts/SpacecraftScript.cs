...
public class SpacecraftScript : MonoBehaviour
{
  ...
  {
    if (Physics.Raycast(DockingPortCam.transform.position, DockingPortCam.transform.forward, 100f) && DockingPortCam.transform.up.y > 0.6) // Casts a raycast checking if it hit the space stations docking port and also if the y rotation of the docking port camera is greater than 0.6
    {
